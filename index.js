...
HaxballJS.then((HBInit) => {
  ...
  function discord_scores_log(
    ...
    for (let i = 0; i < game.goals.length; i++) {
      ...
      if (game.goals[i].assist === "own") {
        ...
        fields[game.goals[i].team == 1 ? 0 : 1].value +=
          "```" +
          `${getTime(game.goals[i])} ⚽ Goal: \n${getPlayerRank(playerauth)} ${
            game.goals[i].striker.name
          }, 👟 Assist: ${game.goals[i].assist.name}` +
          "```\n";
      } else {
        fields[game.goals[i].team == 1 ? 0 : 1].value +=
          "```" +
    ...
    try {
      ...
      console.error("No write permissions for:", replayDirectory, err.message);
      return;
    }
  ...
  function updateRoleOnPlayerIn() {
    ...
    if (inChooseMode) {
      ...
      if (getSpecList(teamR.length <= teamB.length ? teamR[0] : teamB[0]) == -1)
  ...
  room.onPlayerChat = function (player, message) {
    ...
    if (["!help"].includes(message[0].toLowerCase())) {
      ...
      player.admin
        ...
        : null;
    } else if (["!update"].includes(message[0].toLowerCase()) && player.admin) {
      setTimeout((_) => {
        room.sendAnnouncement(
    ...
    if (message[0].toLowerCase().startsWith("@vip")) {
      ...
      if (index !== -1) {
        ...
        if (player.admin) {
          ...
          log_cmd_used(player, "⚜️ vip", getAuth(player), playerReceiving.name);
        } else {
          room.sendAnnouncement(
            "You Need to be Admin!!",
            player.id,
        ...
        room.sendAnnouncement("Player not found.", player.id, 0x309d2b, "bold");
      }
      return false;
    }
    ...
    if (message[0].toLowerCase().startsWith("@gay")) {
      ...
      if (index !== -1) {
        ...
        if (player.admin) {
          ...
          log_cmd_used(player, "🏳️‍🌈 gay", getAuth(player), playerReceiving.name);
        } else {
          room.sendAnnouncement(
            "You Need to be Admin!!",
            player.id,
    ...
    if (message[0].toLowerCase().startsWith("@unrole")) {
      ...
      room.sendAnnouncement(
        player.name + (getAFK(player) ? " it's AFK! 😴" : " is no longer AFK!"),
        null,
    ...
    if (["!edit"].includes(message[0].toLowerCase())) {
      ...
      if (player.admin && message.length >= 2) {
        if (message[1] == "all") {
          ...
          room.sendChat(
            room.getPlayer(Number.parseInt(message[1])).name + " was demutated!"
          );
        } else if (Number.isNaN(Number.parseInt(message[1]))) {
          if (message[1].length > 1 && message[1][0] == "#") {
            message[1] = message[1].substring(1, message[1].length);
    ...
    if (localStorage.getItem(getAuth(player))) {
      ...
      if (stato >= 10000) {
        ...
        let rawdata = fs.readFileSync(path);
        let jsonData = JSON.parse(rawdata);
        let index = jsonData.findIndex((item) => item.auth === getAuth(player));
        if (index !== -1) {
  ...
  function readgameodds(redteam, blueteam) {
    ...
    room.sendAnnouncement(
      `Team odds for bets: Red(${redodds.toFixed(2)}), Blue(${blueodds.toFixed(
        2
      )})`,
      null,
      0x73ec59,
